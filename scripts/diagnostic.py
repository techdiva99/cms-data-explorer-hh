...
def test_database():
    ...
    if not os.path.exists('data/processed/cms_homehealth.db'):
...
def test_analytics():
    ...
    try:
        ...
        # Test a simple query using the new connection method
        conn = analytics._get_connection()
        states = conn.execute("SELECT DISTINCT state FROM providers LIMIT 5").fetchall()
